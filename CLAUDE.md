...
**Raw data manifest**: `docs/wiki/raw_data/` is the catalog of every source under `data/raw_data/` (15 databases + ~70 studies). Per-source pages document citation, methodology, caveats, fidelity (1–5), and a link to the standardizer. Counts are auto-generated by `scripts/analysis/build_raw_data_manifest.py --embed`. Curators should consult this before integrating new sources.

**Standardized data manifest**: `docs/wiki/standardized_data/` is the catalog of post-standardization output under `data/standardized_again/` (18 buckets, 5.67B rows, 1.33B dropped). Per-bucket pages document the standardizer's column-map, drop semantics, column coverage, and yield vs raw input. Counts are auto-generated by `scripts/analysis/build_standardized_manifest.py --embed`. Use this manifest to debug standardization quality issues (drop rates, sparse columns).

**Deduplicated data manifest**: `docs/wiki/deduplicated_data/` is the catalog of Stage 2 output under `data/deduplicated_again/` (1.44B unique deduped rows, 1.46B exploded across 31 subset_keys × per-subset order_key permutations, plus MHC-pseudosequence enrichment). Per-subset pages document the populated columns, combination vs permutation count, order_keys, and which training task the subset feeds. Counts are auto-generated by `scripts/analysis/build_deduplicated_manifest.py --embed`. Use this manifest to pick which subset_key partition feeds a given trainer.
...
| `tra_cdr1` | TCR alpha CDR1 amino acid sequence |
| `tra_cdr2` | TCR alpha CDR2 amino acid sequence |
| `tra_cdr3` | TCR alpha CDR3 amino acid sequence |
| `tra_full` | Full-length TCR alpha chain amino acid sequence (including V/J regions) |
| `trb` | TCR beta chain CDR3 amino acid sequence |
| `trbv_gene` | TCR beta V gene |
| `trbd_gene` | TCR beta D gene |
| `trbj_gene` | TCR beta J gene |
| `trb_cdr1` | TCR beta CDR1 amino acid sequence |
| `trb_cdr2` | TCR beta CDR2 amino acid sequence |
| `trb_cdr3` | TCR beta CDR3 amino acid sequence |
| `trb_full` | Full-length TCR beta chain amino acid sequence (including V
| `peptide` | Peptide/epitope sequence |
| `mhc_one` | MHC class I allele |
| `mhc_two` | MHC class II allele |
| `mhc_one_allele` | MHC class I allele (normalized to standard HLA format) |
| `mhc_two_allele` | MHC class II allele (normalized to standard HLA format) |
