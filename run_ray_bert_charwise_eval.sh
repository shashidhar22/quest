#!/bin/bash
# Evaluate BERT with char-wise tokenized data
# This script evaluates BERT model on character-wise tokenized dataset

set -e  # Exit on any error

echo "🚀 Starting BERT char-wise evaluation..."
...
# Copy the char-wise tokenized data from S3
echo "Copying BERT char-wise tokenized data from S3..."
ray exec config/ray/ray_protbert_eval.yaml "
if aws s3 ls s3://fh-pi-warren-h-eco/quest/hf_final/bert_char_tok/ --region us-west-2 > /dev/null 2>&1; then
  echo 'Found BERT char-wise data, downloading...'
  aws s3 cp --recursive s3://fh-pi-warren-h-eco/quest/hf_final/bert_char_tok /home/ubuntu/quest/data/bert_char_tok --quiet
  echo '✅ BERT char-wise data copy successful'
elif aws s3 ls s3://fh-pi-warren-h-eco/quest/hf_final/bert_charwise/ --region us-west-2 > /dev/null 2>&1; then
  echo 'Found BERT char-wise data (alternative path), downloading...'
  aws s3 cp --recursive s3://fh-pi-warren-h-eco/quest/hf_final/bert_charwise /home/ubuntu/quest/data/bert_char_tok --quiet
  echo '✅ BERT char-wise data copy successful'
else
  echo '❌ BERT char-wise data not found. Available datasets:'
  aws s3 ls s3://fh-pi-warren-h-eco/quest/hf_final/ --region us-west-2
  exit 1
fi
"
...
echo "🎯 Submitting BERT char-wise evaluation job..."
ray job submit \
  ...
  -- python3 scripts/ray_evaluator.py \
      --raw_dataset_dir /home/ubuntu/quest/data/bert_char_tok \
      --model_name bert-base-cased \
      --batch_size 64 \
      --max_length 256 \
      --mlm_prob 0.15 \
      --wandb_project quest-bert-charwise-eval \
      --checkpoint_dir /home/ubuntu/quest/checkpoints \
      --resume_from_checkpoint

# 7. Show job status and instructions
echo "🧹 BERT char-wise evaluation job submitted successfully!"
...
echo ""
echo "✅ BERT char-wise evaluation is running independently on AWS!"