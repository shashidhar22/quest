        ...
        if study_category.is_dir() and study_category.name not in ignore:
            ...
            for file_path in tqdm(tcr_files, desc=f"Processing files in {study_category.name}", leave=False):
                ...
                if file_type == "contigs":
                    mri_table, sequence_table = PairedFileParser(file_path, test=False).parse()
                elif file_type == "clonotypes":
                    mri_table, sequence_table = PairedFileParser(file_path, test=False).parse()
                elif file_type == "airr":
                    mri_table, sequence_table = PairedFileParser(file_path, test=False).parse()
                elif file_type == "misc":
                    mri_table, sequence_table = MiscFileParser(file_path, config_path, test=False).parse()
                elif file_size > 0:
                    mri_table, sequence_table = BulkFileParser(file_path, config_path, test=False).parse()
                else:
                ...
                if mri_table is not None and sequence_table is not None:
                    ...
                    sequence_order = ['trav_gene', 'traj_gene', 'trad_gene',
                                      'tra', 'trbv_gene', 'trbj_gene', 'trbd_gene',
                                      'trb', 'sequence']
                    mri_table = mri_table[mri_order]
                    sequence_table = sequence_table[sequence_order]
...
def parse_database(config_path, output_path):
    ...
    mri_table = mri_table.astype("string[pyarrow]")
    sequence_table = sequence_table.astype("string[pyarrow]")
    with ProgressBar():
