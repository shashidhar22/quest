...
class ParallelStreamAnalyzer:
    ...
    def __init__(self, parsed_dir: str, temp_dir: str = '/mnt/ephemeral', n_parallel_fields: int = 6, use_seq: bool = False):
    ...
    def process_field_worker(self, args):
        ...
        return field_name, dataset_unique, dict(folder_unique)
        ...
        for i in range(0, len(all_fields), self.n_parallel_fields):
            ...
            with ProcessPoolExecutor(max_workers=self.n_parallel_fields) as executor:
                ...
                for future in as_completed(futures):
                    field_name, dataset_unique, folder_unique = future.result()
    ...
    def generate_report(self, folder_stats, dataset_stats, output_file: str):
        ...
        with open(output_file, 'w') as f:
            ...
            for field in ['tra', 'trb', 'tra_junction_aa', 'trb_junction_aa', 
                ...
                key = f'unique_{field}'
                if key in dataset_stats and dataset_stats[key] > 0:
                    f.write(f"  {field:18s}: {dataset_stats[key]:,} unique\n")
            
            f.write("\nPeptide/MHC Single Fields:\n")
            for field in ['peptide', 'mhc_one', 'mhc_two', 'mhc_one_id', 'mhc_two_id']:
                key = f'unique_{field}'
                if key in dataset_stats and dataset_stats[key] > 0:
                    f.write(f"  {field:15s}: {dataset_stats[key]:,} unique\n")
            
            f.write("\nTCR Combinations:\n")
            for combo in ['tra_trav', 'trb_trbv', 'pairs']:
                key = f'unique_{combo}'
                if key in dataset_stats and dataset_stats[key] > 0:
                    f.write(f"  {combo:20s}: {dataset_stats[key]:,} unique\n")
            
            f.write("\nPeptide/MHC Combinations:\n")
            for combo in ['peptide_mhc_one', 'peptide_mhc_two', 'peptide_mhc_one_mhc_two']:
                key = f'unique_{combo}'
                if key in dataset_stats and dataset_stats[key] > 0:
                    f.write(f"  {combo:25s}: {dataset_stats[key]:,} unique\n")
            
            f.write("\nTCR + Peptide/MHC Combinations:\n")
            for combo in ['tra_peptide_mhc_one', 'tra_peptide_mhc_two', 'tra_peptide_mhc_one_mhc_two',
                         'trb_peptide_mhc_one', 'trb_peptide_mhc_two', 'trb_peptide_mhc_one_mhc_two']:
                key = f'unique_{combo}'
                if key in dataset_stats and dataset_stats[key] > 0:
                    f.write(f"  {combo:30s}: {dataset_stats[key]:,} unique\n")
            
            f.write("\nFull Combinations:\n")
            for combo in ['tra_trb_peptide_mhc_one', 'tra_trb_peptide_mhc_two', 'tra_trb_peptide_mhc_one_mhc_two']:
                key = f'unique_{combo}'
                if key in dataset_stats and dataset_stats[key] > 0:
                    f.write(f"  {combo:35s}: {dataset_stats[key]:,} unique\n")
