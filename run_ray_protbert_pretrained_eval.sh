#!/bin/bash
# Evaluate ProtBERT out of the box (pretrained)
# This script evaluates the pretrained ProtBERT model without fine-tuning

set -e  # Exit on any error

echo "🚀 Starting ProtBERT pretrained evaluation..."
...
echo "🎯 Submitting ProtBERT pretrained evaluation job..."
ray job submit \
  ...
  -- python3 scripts/ray_evaluator.py \
      ...
      --max_length 256 \
      --mlm_prob 0.15 \
      --wandb_project quest-protbert-pretrained-eval \
      --checkpoint_dir /home/ubuntu/quest/checkpoints \
      --resume_from_checkpoint

# 7. Show job status and instructions
echo "🧹 ProtBERT pretrained evaluation job submitted successfully!"
...
echo ""
echo "✅ ProtBERT pretrained evaluation is running independently on AWS!"